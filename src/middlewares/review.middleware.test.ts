...
// Jest 모킹 함수로 타입 캐스팅
const mockFindUnique = mockPrisma.estimateRequest
  .findUnique as jest.MockedFunction<
  typeof mockPrisma.estimateRequest.findUnique
>;
const mockFindFirst = mockPrisma.review.findFirst as jest.MockedFunction<
  typeof mockPrisma.review.findFirst
>;
const mockCreate = mockPrisma.review.create as jest.MockedFunction<
  typeof mockPrisma.review.create
>;
  ...
  describe("EstimateRequest 상태 변경 시 리뷰 생성", () => {
    it("EstimateRequest가 COMPLETED로 변경되고 수락된 견적이 있으면 리뷰를 생성한다", async () => {
      ...
      mockFindUnique.mockResolvedValue(
        mockEstimateRequest as Prisma.EstimateRequestGetPayload<{
