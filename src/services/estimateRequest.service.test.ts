...
>;
const mockParseAddress = parseAddress as jest.MockedFunction<
  typeof parseAddress
>;
...
describe("EstimateRequestService", () => {
  ...
  describe("checkUserType", () => {
    it("사용자 타입을 올바르게 반환해야 한다", async () => {
      ...
      mockEstimateRequestRepository.checkUserType = jest
        .fn()
        .mockResolvedValue(mockUserTypeResult);

      // Act
      const result = await service.checkUserType(userId);

      // Assert
      expect(mockEstimateRequestRepository.checkUserType).toHaveBeenCalledWith(
        userId
      );
    ...
    it("레포지토리 에러를 올바르게 전파해야 한다", async () => {
      ...
      // Act & Assert
      await expect(service.checkUserType(userId)).rejects.toThrow(
        "사용자를 찾을 수 없습니다."
      );
      expect(mockEstimateRequestRepository.checkUserType).toHaveBeenCalledWith(
        userId
  ...
  describe("hasPendingRequest", () => {
    it("진행중인 요청이 있으면 true를 반환해야 한다", async () => {
      // Arrange
      const userId = "test-user-id";
      mockEstimateRequestRepository.hasPendingRequest = jest
        .fn()
      ...
      // Assert
      expect(
        mockEstimateRequestRepository.hasPendingRequest
  ...
  describe("hasEstimateFromMover", () => {
    it("기사님이 견적을 제출했으면 true를 반환해야 한다", async () => {
      // Arrange
      const userId = "test-user-id";
      mockEstimateRequestRepository.hasEstimateFromMover = jest
        .fn()
        .mockResolvedValue(true);
      ...
      // Assert
      expect(
        mockEstimateRequestRepository.hasEstimateFromMover
      ).toHaveBeenCalledWith(userId);
      expect(result).toBe(true);
    });
  ...
  describe("getActiveEstimateRequestByUserId", () => {
    it("활성 견적 요청을 성공적으로 반환해야 한다", async () => {
      ...
      mockEstimateRequestRepository.getActiveEstimateRequestByUserId = jest
        .fn()
        .mockResolvedValue(mockActiveRequest);
      ...
      // Assert
      expect(
        mockEstimateRequestRepository.getActiveEstimateRequestByUserId
      ).toHaveBeenCalledWith(userId);
  ...
  describe("createEstimateRequest", () => {
    ...
    it("성공적으로 견적 요청을 생성해야 한다", async () => {
      ...
      mockParseAddress
        .mockReturnValueOnce(mockParsedDeparture)
        .mockReturnValueOnce(mockParsedArrival);
      ...
      mockEstimateRequestRepository.createEstimateRequest = jest
        .fn()
      ...
      expect(
        mockEstimateRequestRepository.findOrCreateAddress
      ).toHaveBeenCalledTimes(2);
      expect(
        mockEstimateRequestRepository.findOrCreateAddress
      ).toHaveBeenCalledWith(mockParsedDeparture);
      expect(
        mockEstimateRequestRepository.findOrCreateAddress
      ).toHaveBeenCalledWith(mockParsedArrival);
      expect(
        mockEstimateRequestRepository.createEstimateRequest
    ...
    it("주소 처리 중 에러가 발생하면 에러를 전파해야 한다", async () => {
      ...
      // Act & Assert
      await expect(service.createEstimateRequest(validParams)).rejects.toThrow(
        "주소 처리 중 오류 발생"
      );
    });
  ...
  describe("updateActiveEstimateRequest", () => {
    ...
    it("성공적으로 견적 요청을 수정해야 한다", async () => {
      ...
      mockEstimateRequestRepository.getEstimateRequestById = jest
        .fn()
        .mockResolvedValue(mockCurrentRequest);
      mockParseAddress
        .mockReturnValueOnce(mockParsedDeparture)
        .mockReturnValueOnce(mockParsedArrival);
      mockEstimateRequestRepository.findOrCreateAddress = jest
      ...
      mockEstimateRequestRepository.updateEstimateRequest = jest
        .fn()
        .mockResolvedValue(mockUpdatedRequest);
      mockEstimateRequestRepository.softDeleteAddress = jest
        .fn()
        .mockResolvedValue(undefined);

      // Act
      const result = await service.updateActiveEstimateRequest(
        requestId,
        updateData
      );
      ...
      // Assert
      expect(
        mockEstimateRequestRepository.getEstimateRequestById
      ...
      expect(
        mockEstimateRequestRepository.updateEstimateRequest
      ).toHaveBeenCalledWith(requestId, {
        moveType: "OFFICE",
        moveDate: new Date("2024-12-26"),
        fromAddressId: "new-from-address-id",
      ...
      expect(
        mockEstimateRequestRepository.softDeleteAddress
      ).toHaveBeenCalledTimes(2);
      expect(
        mockEstimateRequestRepository.softDeleteAddress
      ).toHaveBeenCalledWith("old-from-address-id");
      expect(
        mockEstimateRequestRepository.softDeleteAddress
      ).toHaveBeenCalledWith("old-to-address-id");
    ...
    it("견적 요청을 찾을 수 없으면 에러를 던져야 한다", async () => {
      ...
      // Act & Assert
      await expect(
        service.updateActiveEstimateRequest(requestId, updateData)
    ...
    it("출발지만 수정할 때 올바르게 처리해야 한다", async () => {
      ...
      mockEstimateRequestRepository.findOrCreateAddress = jest
        .fn()
        .mockResolvedValue({ id: "new-from-address-id" });
      mockEstimateRequestRepository.updateEstimateRequest = jest
        .fn()
        .mockResolvedValue(mockUpdatedRequest);
      mockEstimateRequestRepository.softDeleteAddress = jest
    ...
    it("도착지만 수정할 때 올바르게 처리해야 한다", async () => {
      ...
      mockEstimateRequestRepository.findOrCreateAddress = jest
        .fn()
        .mockResolvedValue({ id: "new-to-address-id" });
      mockEstimateRequestRepository.updateEstimateRequest = jest
        .fn()
        .mockResolvedValue(mockUpdatedRequest);
      mockEstimateRequestRepository.softDeleteAddress = jest
    ...
    it("이사 종류를 대문자로 변환해야 한다", async () => {
      ...
      // Act
      await service.updateActiveEstimateRequest(
        requestId,
        updateDataWithLowerCase
      );
    ...
    it("description이 undefined일 때도 올바르게 처리해야 한다", async () => {
      ...
      await service.updateActiveEstimateRequest(
        requestId,
        updateDataWithUndefinedDescription
      );
  ...
  describe("cancelActiveEstimateRequest", () => {
    it("성공적으로 견적 요청을 취소해야 한다", async () => {
      ...
      mockEstimateRequestRepository.cancelEstimateRequest = jest
        .fn()
        .mockResolvedValue(mockCancelledRequest);
      ...
      // Assert
      expect(
        mockEstimateRequestRepository.cancelEstimateRequest
      ).toHaveBeenCalledWith(requestId);
    ...
    it("레포지토리 에러를 올바르게 전파해야 한다", async () => {
      ...
      // Act & Assert
      await expect(
        service.cancelActiveEstimateRequest(requestId)
      ).rejects.toThrow("견적 요청을 찾을 수 없습니다.");
      expect(
        mockEstimateRequestRepository.cancelEstimateRequest
      ).toHaveBeenCalledWith(requestId);
